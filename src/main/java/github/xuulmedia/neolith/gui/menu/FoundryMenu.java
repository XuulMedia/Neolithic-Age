...
public class  FoundryMenu extends AbstractContainerMenu {
    ...
    public float fuelProportion() {
        var maxFuel = this.data.get(FoundryBlockEntity.INDEX_MAX_FUEL_LEFT);
        if (maxFuel < 0) {
            return 0f;
        }

        return (float) this.data.get(FoundryBlockEntity.INDEX_FUEL_LEFT) / (float) maxFuel;
    }

    public float progressProportion() {
        var maxProgress = this.data.get(FoundryBlockEntity.INDEX_MAX_PROGRESS);
        if (maxProgress < 0) {
            return 0f;
        }

        return (float) this.data.get(FoundryBlockEntity.INDEX_PROGRESS) / (float) maxProgress;
    }

    public float heatProportion() {
        return (float) this.data.get(FoundryBlockEntity.INDEX_HEAT) / (float) FoundryBlockEntity.MAX_HEAT;
    }

    public int heat() {
        return this.data.get(FoundryBlockEntity.INDEX_HEAT);
    }

    public int targetHeat() {
        return this.data.get(FoundryBlockEntity.INDEX_TARGET_HEAT);
    }

    public boolean isBurningFuel() {
        return this.data.get(FoundryBlockEntity.INDEX_MAX_FUEL_LEFT) >= 0;
    }
