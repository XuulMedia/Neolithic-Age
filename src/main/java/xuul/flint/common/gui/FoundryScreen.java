...
public class FoundryScreen extends AbstractContainerScreen<FoundryMenu> {
    ...
    protected void renderTooltip(PoseStack ps, int mx, int my) {
        ...
        if (mx >= x + 45 && mx <= x + 50 && my >= y + 22 && my <= y + 63) {
            this.renderTooltip(ps, new TranslatableComponent("container.flint.foundry.heat", this.menu.heat()), mx, my);
        } else if (mx >= x + 56 && mx <= x + 70 && my >= y + 36 && my <= y + 50 && this.menu.isBurningFuel()) {
            this.renderTooltip(ps,
                new TranslatableComponent("container.flint.foundry.heat.target", this.menu.targetHeat()),
                mx, my);
        } else if (mx >= x + 76 && mx <= x + 87 && my >= y + 19 && my <= y + 30 && this.isTooColdToCook()) {
            this.renderTooltip(ps,
                new TranslatableComponent("container.flint.foundry.heat.too_cold", this.heatReqdToCookInput),
                mx, my);
            ...
            && this.hoveredSlot != null && this.hoveredSlot.index == FoundryMenu.FUEL_SLOT) {
            var fuelTemp = this.menu.maxHeatOf(this.menu.getCarried());
            if (fuelTemp != null) {
                this.renderTooltip(ps,
                    new TranslatableComponent("container.flint.foundry.heat.target", fuelTemp),
                    mx, my);
            }
        }
    }
